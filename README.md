...
[`LOI-PHAT-HIEN.md`](LOI-PHAT-HIEN.md) (9 lỗi, có mã và mức nghiêm trọng) →
...
| **Báo cáo lỗi** — 9 lỗi, có mã, mức nghiêm trọng, bước tái hiện, bằng chứng | [`LOI-PHAT-HIEN.md`](LOI-PHAT-HIEN.md) |
...
## Bộ kiểm thử — 295 ca, kiến trúc BDD ba tầng

```bash
pytest                        # tất cả: 229 passed · 61 xfailed · 3 skipped · 2 failed*
...
\* Ca đỏ duy nhất là cổng nghiệp vụ — **đó là kết luận, không phải test hỏng** (xem dưới).

| Tệp | Ca | Kiểm cái gì | Thời gian |
|---|---|---|---|
| `features/*.feature` | 51 kịch bản | **Gherkin tiếng Việt** — AC1/AC2/AC3 chép gần nguyên văn từ đề, góc nhìn nghiệp vụ, và sổ giới hạn. Ban QLDA đọc được mà không mở code | 12 s |
| `test_kien_truc.py` | 11 | **Fitness function kiến trúc** — feature không được dính từ kỹ thuật, bước Gherkin không được import driver, lớp đối tượng không được chứa assert | 0,2 s |
...
| `test_nop_bai.py` | 27 | **Ràng buộc nghiệm thu của đề**: render báo cáo ra PDF A4 rồi ĐẾM SỐ TRANG THẬT (≤2), canh không mục nào bị cắt cho vừa trang, dấu tiếng Việt không vỡ, và đủ danh sách nộp bài ở mục 06 | 1 s |
...
| `test_du_lieu.py` | 134 | **Mỗi (ảnh × trường) là một ca riêng** — 132 điểm kiểm, có tên riêng trong báo cáo, chạy lẻ được | 33 s |
...
data/
  ...
  nhan/doc_lai_doc_lap.csv   lượt gán nhãn thứ hai (người đọc lại ảnh)
